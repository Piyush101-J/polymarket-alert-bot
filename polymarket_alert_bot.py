...
def fetch_bitcoin_price_events():
    """Fetch events and filter for Bitcoin price prediction events"""
    all_markets = []
    seen_slugs = set()
    
    print("=== FETCHING EVENTS AND FILTERING FOR BITCOIN PRICE MARKETS ===")
    ...
    try:
        ...
        for event in events:
            ...
            # Filter for Bitcoin price events
            # Look for patterns like "Bitcoin above ___" or "Bitcoin price"
            if ("bitcoin" in event_title or "btc" in event_title) and \
               ("above" in event_title or "below" in event_title or "price" in event_title or \
                "over" in event_title or "under" in event_title):
                
                bitcoin_events += 1
                print(f"Found Bitcoin price event: {event.get('title', '')}")
        ...
        print(f"=== FOUND {bitcoin_events} BITCOIN PRICE EVENTS WITH {len(all_markets)} MARKETS ===")
...
def bot_loop():
    ...
    try:
        ...
        markets = fetch_bitcoin_price_events()
        
        if len(markets) == 0:
            print("!!! No Bitcoin price markets found !!!")
            return
        ...
        for m in markets:
            ...
            if opportunity:
                ...
                if rec == "YES":
                    ...
                    message += f"💡 BTC is ${int(distance):,} from target"
                else:
        ...
        print(f"\n=== Checked {len(markets)} markets, Sent {alert_count} alerts ===\n")
...
def run_bot():
    print("=== BOT v10 FINAL - SMART EVENT FILTERING ===")
    
    startup_msg = "🎯 Bot v10 FINAL is LIVE!\n\n"
    startup_msg += "✅ Smart filtering for Bitcoin price events\n"
