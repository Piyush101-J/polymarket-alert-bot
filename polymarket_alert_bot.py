...
# OPTIONAL: Add specific markets manually if you want
MANUAL_MARKETS = [
    # Add any special markets here if needed
    # Example: "https://polymarket.com/market/special-bitcoin-bet"
]
...
def fetch_auto_bitcoin_markets():
    """Auto-discover Bitcoin price prediction markets"""
    all_markets = []
    seen_slugs = set()
    
    print("=== AUTO-DISCOVERING BITCOIN MARKETS ===")
    ...
    try:
        ...
        bitcoin_event_count = 0
        for event in events:
            event_title = event.get("title", "").lower()
            
            # Look for Bitcoin price prediction events
            # Pattern: "Bitcoin above", "Bitcoin hits", "BTC above", etc.
            if ("bitcoin" in event_title or "btc" in event_title) and \
               ("above" in event_title or "hit" in event_title or "reach" in event_title or \
                "exceed" in event_title or "over" in event_title):
                
                bitcoin_event_count += 1
        ...
        print(f"=== AUTO-FOUND {bitcoin_event_count} EVENTS WITH {len(all_markets)} MARKETS ===")
...
def fetch_market_by_slug(slug):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"Manual market {slug} returned {response.status_code}")
            return None
    except Exception as e:
        print(f"Error fetching manual market {slug}: {e}")
        return None
...
def bot_loop():
    ...
    try:
        ...
        # Collect all markets (auto + manual)
        all_markets = []
        
        # 1. Auto-discovered markets
        auto_markets = fetch_auto_bitcoin_markets()
        all_markets.extend(auto_markets)
        
        # 2. Manual markets
        print(f"\n=== CHECKING {len(MANUAL_MARKETS)} MANUAL MARKETS ===")
        for market_url in MANUAL_MARKETS:
        ...
        print(f"\n=== TOTAL MARKETS TO CHECK: {len(all_markets)} ===\n")
        ...
        for m in all_markets:
            ...
            yes_prob, no_prob = extract_probabilities(m)
            target_price = extract_target_price(question)
            
            print(f"\n>>> [{source.upper()}] {question[:60]}")
            if event_title:
        ...
        print(f"\n=== Checked {len(all_markets)} markets, Sent {alert_count} alerts ===\n")
...
def run_bot():
    print("=== BOT v13 ULTIMATE - AUTO + MANUAL HYBRID ===")
    
    startup_msg = "🎯 Bot v13 ULTIMATE is LIVE!\n\n"
    startup_msg += "✅ AUTO mode: Discovers Bitcoin markets daily\n"
    startup_msg += "✅ MANUAL mode: Tracks your specific URLs\n"
    startup_msg += "✅ Settings: 1-42%, $1,500-$4,500\n"
    startup_msg += "✅ No daily updates needed!\n\n"
