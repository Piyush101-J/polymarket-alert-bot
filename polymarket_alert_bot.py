...
def bot_loop():
    ...
    try:
        ...
        for m in markets:
            ...
            print("    Keywords: above=" + str("above" in question.lower()) + " over=" + str("over" in question.lower()) + " below=" + str("below" in question.lower()) + " under=" + str("under" in question.lower()))
            ...
            if "above" not in question.lower() and "over" not in question.lower() and "below" not in question.lower() and "under" not in question.lower():
                print("    SKIP: No price direction keyword found")
                continue
            ...
            if MIN_PROB <= prob <= MAX_PROB:
                ...
                if is_price_in_range(current_btc_price, target_price, is_above_market):
                    ...
                    message += "💰 Current BTC: $" + str(current_btc_price) + "\n"
                    message += "📏 Distance: $" + str(abs(target_price - current_btc_price)) + "\n"
                    ...
                    print("    Price not in alert range")
            else:
                print("    SKIP: Probability " + str(round(prob * 100, 2)) + "% not in range " + str(MIN_PROB * 100) + "%-" + str(MAX_PROB * 100) + "%")
...
def run_bot():
    ...
    if send_alert("🚀 Polymarket BTC Alert Bot v2 is LIVE\n\n✅ Only active markets\n✅ Price: Binance (CoinGecko backup)\n✅ Check interval: 60 seconds\n✅ Alert range: $500-$600 from target\n✅ Probability: 1%-40%"):
