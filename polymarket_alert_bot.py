...
# ADD EVENT SLUGS (not full URLs, just the event part!)
MONITORED_EVENTS = [
    "bitcoin-above-on-february-10",
    "bitcoin-above-on-february-8",
    "bitcoin-above-on-february-9",
    "bitcoin-above-on-february-7",
    "bitcoin-above-on-february-11",
    "bitcoin-above-on-february-12",
    # Add more event slugs as needed
]
...
def fetch_event_by_slug(event_slug):
    """Fetch an entire event (which contains multiple markets)"""
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"    Event {event_slug} returned status {response.status_code}")
            return None
    except Exception as e:
...
def bot_loop():
    ...
    try:
        ...
        print(f"\n=== CHECKING {len(MONITORED_EVENTS)} EVENTS ===\n")
        ...
        for event_slug in MONITORED_EVENTS:
            ...
            if len(markets) == 0:
                print(f"    Event has no markets")
                continue
            
            # Check if event expired by checking first market
            if markets and is_market_expired_or_resolved(markets[0]):
                expired_event_count += 1
                print(f"    SKIP: Event expired ⏰")
                continue
            
            active_event_count += 1
            print(f"    ✅ Event Active: {event_title}")
            ...
            for market in markets:
                ...
                if opportunity:
                    ...
                    message += f"\n\n🔗 https://polymarket.com/event/{event_slug}/{slug}"
        ...
        print(f"\n=== SUMMARY ===")
        print(f"Total Events: {len(MONITORED_EVENTS)}")
        print(f"Active Events: {active_event_count} ✅")
        print(f"Expired Events: {expired_event_count} ⏰")
        print(f"Markets Checked: {total_markets_checked}")
...
def run_bot():
    print("=== BOT v15 FINAL - EVENT-BASED MONITORING ===")
    print(f"Monitoring {len(MONITORED_EVENTS)} events")
    
    startup_msg = "🎯 Bot v15 FINAL is LIVE!\n\n"
    startup_msg += f"✅ Monitoring {len(MONITORED_EVENTS)} events\n"
    startup_msg += "✅ Auto-checks all markets in each event\n"
    startup_msg += "✅ Auto-skips expired events ⏰\n"
    startup_msg += "✅ Settings: 1-42%, $1,500-$4,500\n\n"
    startup_msg += "Ready to catch opportunities!"
