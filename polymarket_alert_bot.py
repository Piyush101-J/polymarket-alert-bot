...
def fetch_markets_and_events():
    """Fetch both regular markets AND event-grouped markets"""
    all_markets = []
    seen_slugs = set()
    
    print("=== FETCHING MARKETS AND EVENTS ===")
    
    # Method 1: Regular markets API
    try:
    ...
    # Method 2: EVENTS API (This is the key!)
    try:
        print("Fetching EVENTS (grouped markets)...")
        ...
        # Events contain multiple markets inside them
        ...
        for event in events:
            ...
            for m in markets_in_event:
                ...
                # Add event title context to market
    ...
    print("=== TOTAL MARKETS (regular + events): " + str(len(all_markets)) + " ===")
    return all_markets
...
def bot_loop():
    ...
    try:
        ...
        for i, m in enumerate(markets[:15], 1):
            ...
            if event_title:
                print(str(i) + ". [EVENT: " + event_title[:40] + "] " + question[:50])
            else:
        ...
        for m in markets:
            ...
            # Check Bitcoin in question OR event title
            full_text = (question + " " + event_title).lower()
            if "bitcoin" not in full_text and "btc" not in full_text:
                continue
            
            bitcoin_count += 1
            print("\n>>> BITCOIN MARKET: " + question)
            if event_title:
                print("    Part of event: " + event_title)
            ...
            if not is_above_market:
                is_above_market = "below" not in full_text and "under" not in full_text
            ...
            if yes_prob is None or no_prob is None:
                print("    SKIP: Missing probability")
                continue
            
            if target_price is None:
                print("    SKIP: No target price")
                continue
            ...
            print("    Prob in range? " + str(MIN_YES_PROB <= yes_prob <= MAX_YES_PROB))
            ...
            if opportunity:
                ...
                if slug in alerted_markets:
                    ...
                    if time_since < ALERT_COOLDOWN:
                        print("    SKIP: Alerted " + str(int(time_since/60)) + " min ago")
                        continue
                ...
                if rec == "YES":
                    ...
                    message += "💡 $" + str(int(distance)) + " from target"
                else:
                    message += "🧠 BUY NO ✅\n"
                    message += "📈 Confidence: " + conf
...
def run_bot():
    print("=== POLYMARKET BOT v6 FINAL - EVENTS API ===")
    
    startup_msg = "🚀 Bot v6 FINAL is LIVE!\n\n"
    startup_msg += "✅ Now checking EVENTS API!\n"
    startup_msg += "✅ Will find grouped Bitcoin markets\n"
    startup_msg += "✅ Settings: 1-42%, $1.5k-$4.5k\n"
    startup_msg += "✅ Checks every 60 seconds\n\n"
