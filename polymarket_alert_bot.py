...
def fetch_bitcoin_price_events():
    """Fetch ALL events and show their titles for debugging"""
    all_markets = []
    seen_slugs = set()
    
    print("=== FETCHING EVENTS (DEBUG MODE) ===")
    ...
    try:
        ...
        # DEBUG: Show first 30 event titles
        print("=== EVENT TITLES (First 30) ===")
        for i, event in enumerate(events[:30], 1):
            title = event.get("title", "NO TITLE")
            slug = event.get("slug", "NO SLUG")
            market_count = len(event.get("markets", []))
            print(f"{i}. [{market_count} markets] {title[:80]}")
        if len(events) > 30:
            print(f"... and {len(events) - 30} more events")
        print("=== END EVENT TITLES ===\n")
        
        bitcoin_events = 0
        for event in events:
            event_title = event.get("title", "").lower()
            
            # Very loose filter - just check for bitcoin/btc
            if "bitcoin" in event_title or "btc" in event_title:
                
                bitcoin_events += 1
                print(f">>> Found Bitcoin event: {event.get('title', '')}")
                
                markets_in_event = event.get("markets", [])
                print(f"    Contains {len(markets_in_event)} markets")
        ...
        print(f"\n=== FOUND {bitcoin_events} BITCOIN EVENTS WITH {len(all_markets)} MARKETS ===")
...
def bot_loop():
    ...
    try:
        ...
        if len(markets) == 0:
            print("!!! No Bitcoin markets found - check event titles above !!!")
            return
        ...
        print(f"\n=== Checked {len(markets)} markets, Alerts: {alert_count} ===\n")
...
def run_bot():
    print("=== BOT v11 DEBUG - EVENT TITLE LOGGING ===")
    
    startup_msg = "Bot v11 DEBUG\nCheck logs to see event titles"
    ...
    print("\n=== DEBUG RUN COMPLETE ===")
    print("Check logs above to see what events exist!")
    print("="*60)
